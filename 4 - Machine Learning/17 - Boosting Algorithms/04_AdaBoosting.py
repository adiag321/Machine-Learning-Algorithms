...
def adaboost_model(X_train, X_test, y_train, y_test, feature_names=None):
    """
    Parameters:
    -----------
    X_train : array-like
    ...
    ada_clf = AdaBoostClassifier(
        ...
        learning_rate=0.1,             # Shrinks contribution of each weak learner
        random_state=42,               # Reproducibility
        algorithm='SAMME'            # Uses probability estimates for more stable boosting
    )
    ...
    y_pred_proba_train = ada_clf.predict_proba(X_train)[:, 1]
    y_pred_proba_test = ada_clf.predict_proba(X_test)[:, 1]
    ...
    cv_scores = cross_val_score(ada_clf, X_train, y_train, cv=StratifiedKFold(n_splits=5), scoring='f1')
    ...
    feature_imp = ada_clf.feature_importances_
    top_n = 10
...
adaboost_model(X_train_scaled, X_test_scaled, y_train, y_test, feature_names=feature_names)